...
//using JsonEntity = RA.Models.Json.Credential;
...
{
	public class CredentialsImport : RegistryImport
	{
		int entityTypeId = CodesManager.ENTITY_TYPE_CREDENTIAL;
		ImportCredential entityImportMgr = new ImportCredential();
		ImportServiceHelpers importMgr = new ImportServiceHelpers();
		static string thisClassName = "CredentialsImport";
		...
		public string Import( string startingDate, string endingDate, int maxRecords, bool downloadOnly = false )
		{
			...
			while ( pageNbr > 0 && !isComplete )
			{
				...
				if ( list == null || list.Count == 0 )
				{
					...
					if ( pageNbr == 1 )
					{
						importNote = "Credentials: No records where found for date range ";
				...
				foreach ( ReadEnvelope item in list )
				{
                    ...
                    status.ValidationGroup = "Credential Import";
					importError = "";
					importSuccessfull = false;

					try
					{
						Console.WriteLine( string.Format( "{0}. Credential EnvelopeIdentifier {1} ", cntr, item.EnvelopeIdentifier ) );
				...
				{
					isComplete = true;
					LoggingHelper.DoTrace( 2, string.Format( "ImportCredentials EARLY EXIT. Completed {0} records out of a total of {1} ", cntr, pTotalRows ) );
