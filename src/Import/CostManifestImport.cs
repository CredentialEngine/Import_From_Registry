...
//using JsonEntity = RA.Models.Json.CostManifest;
...
{
	public class CostManifestImport : RegistryImport
	{
		int entityTypeId = CodesManager.ENTITY_TYPE_COST_MANIFEST;
		ImportCostManifests entityImportMgr = new ImportCostManifests();
		ImportServiceHelpers importMgr = new ImportServiceHelpers();
		static string thisClassName = "CostManifestImport";
		...
		public string Import( string startingDate, string endingDate, int maxRecords, bool downloadOnly = false )
		{
			...
			string type = "cost_manifest_schema";
			...
			while ( pageNbr > 0 && !isComplete )
			{
				...
				if ( list == null || list.Count == 0 )
				{
					...
					if ( pageNbr == 1 )
					{
						importNote = " Cost Manifests: No records where found for date range ";
				...
				foreach ( ReadEnvelope item in list )
				{
					...
					try
					{
						Console.WriteLine( string.Format( "{0}. Cost Manifest EnvelopeIdentifier {1} ", cntr, item.EnvelopeIdentifier ) );
				...
				{
					isComplete = true;
					LoggingHelper.DoTrace( 2, string.Format( "CostManifestImport EARLY EXIT. Completed {0} records out of a total of {1} ", cntr, pTotalRows ) );
