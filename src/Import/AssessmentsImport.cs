...
{
    public class AssessmentsImport : RegistryImport
    {

		int entityTypeId = CodesManager.ENTITY_TYPE_ASSESSMENT_PROFILE;
		ImportAssessment entityImportMgr = new ImportAssessment();
		ImportServiceHelpers importMgr = new ImportServiceHelpers();
		static string thisClassName = "AssessmentsImport";
		...
		public string Import( string startingDate, string endingDate, int maxRecords, bool downloadOnly = false )
		{
            ...
            while ( pageNbr > 0 && !isComplete )
            {
				...
				{
                    isComplete = true;
                    LoggingHelper.DoTrace( 2, thisClassName + string.Format( ".ImportCredentials EARLY EXIT. Completed {0} records out of a total of {1} ", cntr, pTotalRows ) );
