...
{
	public class LearningOpportuniesImport : RegistryImport
	{
		int entityTypeId = CodesManager.ENTITY_TYPE_LEARNING_OPP_PROFILE;
		ImportLearningOpportunties entityImportMgr = new ImportLearningOpportunties();
		ImportServiceHelpers importMgr = new ImportServiceHelpers();
		static string thisClassName = "LearningOpportuniesImport";
		...
		public string Import( string startingDate, string endingDate, int maxRecords, bool downloadOnly = false )
		{
			...
			string type = "learning_opportunity_profile";
			int entityTypeId = CodesManager.ENTITY_TYPE_LEARNING_OPP_PROFILE;
			int pageNbr = 1;
			int pageSize = 25;
			string importError = "";
