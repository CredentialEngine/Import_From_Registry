...
namespace Import.Services
{
    public class ResolutionServices
    {
        ...
        public static Guid ResolveEntityByRegistryAtIdToGuid( string property, string referencedAtId, int entityTypeId, ref SaveStatus status, ref bool isResolved, string parentCTID = "" )
        {
            ...
            if ( entityTypeId == 0 )
            {
                //not sure why there is no entityTypeId
                //could do another look up?
                var ctdlType = string.Empty;

                //unfortunately this method strips the namespace? Need an alternate method, or configure to not string the namespace?
                var resource = RegistryServices.GetResourceByCtid( ctid, ref ctdlType, ref statusMsg, UtilityManager.GetAppKeyValue( "defaultCommunity" ) );
                if ( resource != null && string.IsNullOrEmpty( resource ) )
                {
                    entityTypeId = CodesManager.Codes_GetEntityTypeId( ctdlType );
                }
                else
                {
                    var msg = string.Format( thisClassName + ".ResolveEntityByRegistryAtIdToGuid. Very unlikely error. A resource was not found for property: '{0}' CTID: '{1}'.", property, ctid );
                    status.AddError( msg );
        ...
        public static int HandleAddPendingRecords( string property, int entityTypeId, string ctid, string referencedAtId, ref Guid entityUid, ref SaveStatus status, string parentCTID = "" )
        {
            ...
            switch ( entityTypeId )
            {
                ...
                case 38:
                    ...
                    break;
                default:
                    status.AddError( $"{thisClassName}.HandleAddPendingRecords EntityTypeId of {entityTypeId} is not being handled yet." );
                    LoggingHelper.DoTrace( 4, string.Format( "ResolutionServices.HandleAddPendingRecords. The property: '{0}' with an entityTypeId of: {1} could not be resolved. URI: {2}", property, entityTypeId, referencedAtId ) );
                    break;
            }
