...
using DBResource = workIT.Data.Tables.Entity_Job;
using EntityContext = workIT.Data.Tables.workITEntities;
using ViewContext = workIT.Data.Views.workITViews;
using RelatedResourceManager = workIT.Factories.JobManager;
...
{
    public class Entity_JobManager : BaseFactory
    {
        static string thisClassName = "Entity_JobManager";
        static string ResourceType = "Entity_Job";
        static string RelatedResourceType = "Job";
        static string ResourceLabel = "Job";
        public RelatedResourceManager relatedResourceManager = new RelatedResourceManager();
        #region Entity Job Persistance ===================

        /// <summary>
        /// Add an Job to a parent (typically a stub was created, so can be associated before completing the full profile)
        /// </summary>
            ...
            )
        {
            ...
            using ( var context = new EntityContext() )
            {
                ...
                try
                {
                    ...
                    efEntity = context.Entity_Job
                            .FirstOrDefault( s => s.EntityId == parent.Id && s.JobId == resourceId && s.RelationshipTypeId == relationshipTypeId );
        ...
        public bool DeleteAll( Entity parent, ref SaveStatus status )
        {
            ...
            using ( var context = new EntityContext() )
            {
                ...
                var results = context.Entity_Job
                            .Where( s => s.EntityId == parent.Id )
                            .OrderBy( s => s.JobProfile.Name )
                            .ToList();
                ...
                {
                    //if a reference, delete actual Job if not used elsewhere
                    if ( item.JobProfile != null && item.JobProfile.EntityStateId == 2 )
                    {
                        //do a getall. If only one, delete it.
                        var exists = context.Entity_Job
                            .Where( s => s.JobId == item.JobId )
                            .ToList();
                        if ( exists != null && exists.Count() == 1 )
                        {
                            ...
                            //actually this delete will probably also delete the Entity_Job
                            //new JobManager().Delete( item.JobId, ref statusMsg );
                            //continue;
                        }
                    }
                    context.Entity_Job.Remove( item );
                    count = context.SaveChanges();
                    if ( count > 0 )
                    {
                    ...
                    int relationshipTypeId = 1 )
        {
            ...
            try
            {
                using ( var context = new EntityContext() )
                {
                    List<DBResource> results = context.Entity_Job
                    ...
                    if ( results != null && results.Count > 0 )
                    {
                        foreach ( DBResource item in results )
                        {
                            ...
                            if ( item.JobProfile != null && item.JobProfile.EntityStateId > 1 )
                            {
                                if ( forProfilesList )
                                {
                                    //TODO - add a minimal mapper
                                    RelatedResourceManager.MapFromDB( item.JobProfile, entity, false );
                                    list.Add( entity );

                                }
                                else
                                {
                                    //if ( CacheManager.IsJobAvailableFromCache( item.JobId, ref entity ) )
                                    //{
                                    //    list.Add( entity );
                                    //}
                                    //else
